...
/* Created on:     28/7/2023 11:32:51 am                        */
/*==============================================================*/
...
if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('BOOK_DETAIL') and o.name = 'FK_BOOK_DET_WRITTEN_B_AUTHOR')
alter table BOOK_DETAIL
   drop constraint FK_BOOK_DET_WRITTEN_B_AUTHOR
go
...
if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('HAS_VOUCHER') and o.name = 'FK_HAS_VOUC_HAS_VOUCH_ACCOUNT')
alter table HAS_VOUCHER
   drop constraint FK_HAS_VOUC_HAS_VOUCH_ACCOUNT
go

if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('HAS_VOUCHER') and o.name = 'FK_HAS_VOUC_HAS_VOUCH_VOUCHER')
alter table HAS_VOUCHER
   drop constraint FK_HAS_VOUC_HAS_VOUCH_VOUCHER
go
...
if exists (select 1
           ...
           where  id    = object_id('BOOK_DETAIL')
            ...
            and   indid > 0
            and   indid < 255)
   drop index BOOK_DETAIL.WRITTEN_BY_FK
go
...
if exists (select 1
            from  sysindexes
           where  id    = object_id('HAS_VOUCHER')
            and   name  = 'HAS_VOUCHER2_FK'
            and   indid > 0
            and   indid < 255)
   drop index HAS_VOUCHER.HAS_VOUCHER2_FK
go

if exists (select 1
            from  sysindexes
           where  id    = object_id('HAS_VOUCHER')
            and   name  = 'HAS_VOUCHER_FK'
            and   indid > 0
            and   indid < 255)
   drop index HAS_VOUCHER.HAS_VOUCHER_FK
go
...
create table BOOK (
   ...
   BOOK_PRICE           int                  null,
   BOOK_PATH            varchar(50)          null,
   BOOK_FILENAME        varchar(50)          null,
   AVG_RATING           float                null,
   COUNT_RATING         int                  null,
   STOCK                int                  null,
   DISCOUNTED_NUMBER    int                  null,
   ATTRIBUTE_76         int                  null,
...
/* Index: WRITTEN_BY_FK                                         */
/*==============================================================*/
create index WRITTEN_BY_FK on BOOK_DETAIL (
AUTHOR_ID ASC
)
go
...
create table DISTRICT (
   DIST_ID              char(13)             not null,
   PROV_ID              char(13)             not null,
...
create table HAS_VOUCHER (
   EMAIL                varchar(100)         not null,
   VOUCHER_ID           char(13)             not null,
   constraint PK_HAS_VOUCHER primary key (EMAIL, VOUCHER_ID)
)
go

/*==============================================================*/
/* Index: HAS_VOUCHER_FK                                        */
/*==============================================================*/
create index HAS_VOUCHER_FK on HAS_VOUCHER (
EMAIL ASC
)
go

/*==============================================================*/
/* Index: HAS_VOUCHER2_FK                                       */
/*==============================================================*/
create index HAS_VOUCHER2_FK on HAS_VOUCHER (
VOUCHER_ID ASC
)
go
...
create table SHIPPING_ADDRESS (
   ...
   EMAIL                varchar(100)         not null,
   DIST_ID              char(13)             not null,
   WARD_ID              char(13)             not null,
   PROV_ID              char(13)             not null,
   ATTRIBUTE_35         varchar(50)          null,
   constraint PK_SHIPPING_ADDRESS primary key nonclustered (ADD_ID)
)
go
...
create table VOUCHER (
   VOUCHER_ID           char(13)             not null,
   VOUCHER_TYPE_ID      char(3)              not null,
...
alter table BOOK
   add constraint FK_BOOK_HAS_CATEG_CATEGORY foreign key (ID)
...
alter table BOOK_DETAIL
   add constraint FK_BOOK_DET_WRITTEN_B_AUTHOR foreign key (AUTHOR_ID)
...
alter table HAS_VOUCHER
   add constraint FK_HAS_VOUC_HAS_VOUCH_ACCOUNT foreign key (EMAIL)
      references ACCOUNT (EMAIL)
go

alter table HAS_VOUCHER
   add constraint FK_HAS_VOUC_HAS_VOUCH_VOUCHER foreign key (VOUCHER_ID)
...
alter table H_ORDER
   add constraint FK_H_ORDER_HAS_SHIPP_SHIPPING foreign key (ADD_ID)
      references SHIPPING_ADDRESS (ADD_ID)
go
