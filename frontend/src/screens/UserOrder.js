...
function UserOrder() {
  ...
  return (
    <div className='user-order-container'>
        <button className='user-back-button' type='button'>&lt; Back</button>
        <div className='user-order-category'>
            {orderCategory.map((category, index)=>(
                <button className='category-btn' type='button' onClick={()=>setCategory(index)} onBlur={(e)=>{setFocus(e)}}>{category}</button>
            ))}
        </div>
                                            ...
                                            if (index === 0){
                                                ...
                                                return (
                                                    <div className='user-order-product-detail'>
                                                        <a href='/'><img src={product.img} alt={product.book}></img></a>
                                                        <div className='order-product-detail'>
                                                            <h3>{product.book}</h3>
                                                            <p>Giá: {product.price}</p>
                                                            <p>Số lượng: {product.quantity}</p>
                                                        </div>
                                                        <button className='button' type='button'>{buttonName(p[0])}</button>
                                                    </div>
                                                )
                                            }
                                            else{
